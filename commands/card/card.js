    ...
    return this.plugins.Account.exists(msg.author.id)
      ...
      .then(account => {
        ...
        if(account.card.heroes.split('').length < 1){
          return this.plugins.Opendota.card_heroes(account.dota).catch(err => { throw new UserError('opendota', 'errorOpendotaRequest', err ) }).then(results => {
          ...
          return this.plugins.Opendota.card(account.dota).catch(err => {console.log(err);throw new UserError('opendota', 'errorOpendotaRequest', err ) })
            ...
            .then(buffer => this.createMessage(this.config.guild.generated,'',{file : buffer, name : user.username + '_roshan_card.png'}))
            .then(m => msg.reply({embed : {
                description : `${this.locale.replacer(lang.playerCard,{username : user.username})}\n${this.plugins.Account.socialLinks(account)}`,
                image : { url : m.attachments[0].url},
                color : this.config.color}
              }))
            .catch(err => {
