...
module.exports = new Command('deck', {
    ...
    function (msg, args, command) {
        ...
        if(deck){
            ...
            return this.plugins.Artifact.generateDeck(code, args[2] ? args.from(2) : null)
                .then(result => {
                    ...
                    return this.createMessage(this.config.guild.generated, '', { file: result.buffer, name: result.data.code + '.jpg' })
                        .then(m => this.plugins.Artifact.saveDeckIntoCache(result.data.code, result.data.name, msg.author.id, m.attachments[0].url))
                        .then(deck => embedResponse(msg, this, deck))
                })
        }
    })
