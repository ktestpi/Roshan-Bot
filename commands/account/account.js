    ...
    return this.plugins.Opendota.userID(msg, args)
      .then(player => {
        ...
        if (args.length < 2) {
          ...
          if (account.dota) {
            this.plugins.Opendota.account(account.dota).then(result => {
              if (!result.profile) { msg.addReaction(this.config.emojis.default.error); return }; //TODO need register
              this.notifier.accountmodify(`Account modified: **${msg.author.username}** (${msg.author.id})`)
              ...
              \`\n<dota> <dotaID>\n<steam> <steamID>\n<twitch> <twitchID>\n<twitter> <twitterID>`, { dotaID: account.dota, steamID: account.steam, twitchID: account.twitch, twitterID: account.twitter }),
                ...
                return this.cache.profiles.modify(msg.author.id, { profile: account }).then(() => {
                  msg.replyDM({
                    embed: {
