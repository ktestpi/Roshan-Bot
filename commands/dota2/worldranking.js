...
module.exports = new Command('worldranking',{
  ...
  function(msg, args, command){
    const div = args[1] ? args[1].toLowerCase() : this.components.WorldRankingApi.defaultDivision
    const division = this.components.WorldRankingApi.divisions.indexOf(div) > -1 ? div : this.components.WorldRankingApi.defaultDivision
    const lang = this.locale.getUserStrings(msg)
    msg.channel.sendTyping()
    return this.components.WorldRankingApi.get(division).then(r => {
      ...
      return msg.reply({embed : {
        ...
        description :  table.render() + `\n**${lang.divisions}**: \`${this.components.WorldRankingApi.divisions.sort().join(', ')}\``,
