...
module.exports = new Command(['competitive','comp'],{
  ...
  function(msg, args, command){
    ...
    return this.components.Opendota.competitive()
      .then(results => {
        ...
        results[0].slice(0,8).forEach(match => {
          ...
          table += Classes.Table.renderRow([this.components.Bot.parseText(match.radiant_name, 'nf'), victory, this.components.Bot.parseText(match.dire_name, 'nf'), this.components.Bot.parseText(match.league_name, 'nf')], spacesBoard, '\u2002') + ' ' + Markdown.link('https://www.dotabuff.com/matches/' + match.match_id, match.match_id) + '\n';    
        })
        return msg.reply({
          embed: {
